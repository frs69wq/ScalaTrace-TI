...
do
    for class in C
    do
	for cores in 32 16 8 4 2 1
	do
	    for run in I II III IV V
	    do
		echo -n "Replaying $class-$processes-$run for $cores cores... "

		if [ $processes -eq 16 ] && [ $cores -eq 32 ]
		then
		    echo "Skipped"
		    continue
		fi
		...
		echo "$class, $processes, $cores, $realTime, RealTime, $run" >> datasets/simulationsTD-whatif-1.csv
		echo "$class, $processes, $cores, $simTime, SimTime, $run" >> datasets/simulationsTD-whatif-1.csv
		echo "$class, $processes, $cores, $runTime, RunTime, $run" >> datasets/simulationsTD-whatif-1.csv
