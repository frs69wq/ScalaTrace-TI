---
title: "Comp 4b ratios of TI vs TI"
author: "Anshul Gupta"
date: "27/06/2014"
...
## Motive
The idea behind checking the ratios of multiple time independent traces with each other is to see weather the processing is affected by any factors or not.

## Overview
Multiple time independent traces were acquired on graphene cluster for LU benchmark, class C and 16 Processes.  
Their **COMP 4b** global values were parsed using the _scripts/Expand.java_ code
with this format.  
...
where `<A>` is the Time Dependent dataset and `<B>` is the Time Independent dataset.   
Each `d<A>_<B>` has 7 fields:

* xlabel = eventID_subblock
* avgTD = Average of time independent trace A
* minTD = Minimum of time independent trace A
* maxTD = Maximum of time independent trace A
...
## Conclusion
The ratios are almost close to 1, which shows that the CPU processing is almost constant for every execution.