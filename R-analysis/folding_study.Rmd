...
Then we focus on the 50 most time-consuming events in the trace. 

```{r}
times = data.frame(head(time_regular[order(time_regular$COMP.4b.avg, decreasing = TRUE),],50)$COMP.4b.avg, 
                   head(time_2[order(time_2$COMP.4b.avg, decreasing = TRUE),], 50)$COMP.4b.avg,
                   head(time_4[order(time_4$COMP.4b.avg, decreasing = TRUE),], 50)$COMP.4b.avg,
                   head(time_8[order(time_8$COMP.4b.avg, decreasing = TRUE),], 50)$COMP.4b.avg,
                   head(time_16[order(time_16$COMP.4b.avg, decreasing = TRUE),], 50)$COMP.4b.avg, 
                   head(time_32[order(time_32$COMP.4b.avg, decreasing = TRUE),], 50)$COMP.4b.avg)
names(times) <- c(1,2,4,8,16,32)

summary(times)

ggplot(melt(times), aes(variable, value))  + xlab("Folding factor") + ylab("(Folded/Regular) Time Ratio") + geom_boxplot(outlier.shape = NA)+ylim(0,400000)
```
